...
const ViewableItemPage: React.FC = () => {
  ...
  const item = useAppSelector((store) =>
    selectorsDataOfSearchedItems.selectById(store, activeItemId!)
  );
  ...
  return activeItemId === null ? (
    ...
    <GlassElement className="color-text mx-3 p-4 d-flex flex-column justify-content-between gap-5 rounded overflow-auto scrollbar">
      <div className="d-flex flex-column flex-nowrap">
        {titleOfPage.map(
          (dataOfItem, index) =>
            dataOfItem && (
              <p key={index} className="fw-bold fs-3">
                {dataOfItem}
              </p>
            )
        )}
      ...
      <div className="d-flex justify-content-start flex-wrap gap-5">
        ...
        <div
          className="scrollbar py-2 w-100 overflow-auto align-self-center border-top border-bottom"
          style={{ maxHeight: '250px', maxWidth: '900px' }}
        >
          {description ? description : <div style={{ width: '100%', height: '200px' }}></div>}
        </div>
      </div>
    </GlassElement>
  );
